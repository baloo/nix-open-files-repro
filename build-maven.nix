# This is a fork from https://github.com/NixOS/nixpkgs/blob/master/pkgs/build-support/build-maven.nix
{ stdenv, maven, runCommand, writeText, fetchurl, lib, requireFile, writeScript, symlinkJoin }:
...
infoFile: let
  ...
  window = size: f: deps:
  let buckets = lib.range 0 ((builtins.length deps) / size);
  in map 
    (bucket: 
      f
        bucket
        (lib.sublist (bucket * size) size deps)
    )
    buckets;
    
  layer1 = (id: subset:
    runCommand "build-maven-repository-layer1-${toString id}" {} ''
  ...
  layer0 = symlinkJoin {
    name = "build-maven-repository-layer0";
    paths = window 4 layer1 info.dependencies;
  };
